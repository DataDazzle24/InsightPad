...
type ConnectionOption = { id: string; provider: SalesChannelProvider; displayName: string; branchId: string; branchName: string; status: string; authorizationStatus: string; enabled: boolean };
...
type Mapping = {
  id: string; connectionId: string; connectionName: string; provider: SalesChannelProvider; branchName: string; productId: string;
...
type ProductOption = { id: string; name: string; internalCode?: string | null; ean?: string | null };
...
export function SalesChannelsPage() {
  ...
  function openMapping(item?: Mapping) { setMappingModal(item ?? "new"); setMappingForm(item ? { connectionId: item.connectionId, productId: item.productId, externalProductId: item.externalProductId, externalProductName: item.externalProductName ?? "", syncPrice: item.syncPrice, syncStock: item.syncStock, enabled: item.enabled } : blankMapping(options.connections[0]?.id)); }
  ...
  async function saveConnection(event: FormEvent) {
    ...
    try {
      ...
      const result = editing
        ...
        : await executeMutation(mutationRef(dc, "CreateSalesChannelConnection", { provider: connectionForm.provider, branchId: connectionForm.branchId, displayName: connectionForm.displayName.trim(), externalStoreId: connectionForm.externalStoreId.trim() }));
    ...
    } catch (error) { console.error(error); setNotice({ type: "error", text: "Não foi possível salvar. Confirme se a filial já possui esse parceiro." }); } finally { setActing(null); }
  }
  async function saveMapping(event: FormEvent) {
    event.preventDefault();
    if (!mappingForm.connectionId || !mappingForm.productId || !mappingForm.externalProductId.trim()) return setNotice({ type: "error", text: "Selecione conexão e produto e informe o ID externo." });
    setActing("mapping");
    try {
    ...
    } catch (error) { console.error(error); setNotice({ type: "error", text: "Não foi possível salvar. Verifique produto ou ID externo duplicado." }); } finally { setActing(null); }
  }
  ...
  async function sync(connectionId: string) {
    ...
    try {
      ...
      await refresh("Sincronização enfileirada sem bloquear a operação.");
  ...
  return <section className="catalog-page channels-page">
    ...
    {section === "connections" && <Connections data={connections} permission={permission} sort={sort} acting={acting} onSort={changeSort} onEdit={openConnection} onDiagnostics={setDiagnostics} onSync={sync} onRemove={setConfirmRemove} />}
    ...
    {confirmRemove && <RemoveDialog connection={"branchId" in confirmRemove} busy={acting === confirmRemove.id} onClose={() => setConfirmRemove(null)} onConfirm={() => void remove()} />}
    {diagnostics && <Diagnostics connection={diagnostics} onClose={() => setDiagnostics(null)} />}
    {busy && <Loading />}
  </section>;
}

function Readiness() { return <div className="channels-guidance"><span className="material-symbols-rounded">verified_user</span><div><strong>Central segura, pronta para o credenciamento dos parceiros</strong><p>Pedidos, catálogo, comandos, eventos e diagnósticos estão separados por empresa e filial. Tokens e chaves nunca passam pelo navegador; a operação externa só será liberada após homologação oficial.</p></div><span className="channel-readiness">BASE PREPARADA</span></div>; }
...
function Connections({ data, permission, sort, acting, onSort, onEdit, onDiagnostics, onSync, onRemove }: { data: Paged<Connection>; permission?: { canUpdate?: boolean; canDelete?: boolean; canManage?: boolean }; sort: TableSort | null; acting: string | null; onSort: (key: string) => void; onEdit: (item: Connection) => void; onDiagnostics: (item: Connection) => void; onSync: (id: string) => Promise<void>; onRemove: (item: Connection) => void }) {
  return <DataPanel><table><thead><tr><SortableTableHeader label="Canal" sortKey="provider" sort={sort} onChange={onSort} /><SortableTableHeader label="Loja" sortKey="displayName" sort={sort} onChange={onSort} /><SortableTableHeader label="Filial" sortKey="branchName" sort={sort} onChange={onSort} /><th>ID externo</th><SortableTableHeader label="Saúde" sortKey="status" sort={sort} onChange={onSort} /><th>Catálogo</th><SortableTableHeader label="Última sincronização" sortKey="lastSyncedAt" sort={sort} onChange={onSort} /><th>Ações</th></tr></thead><tbody>{data.rows.map((item) => { const health = salesChannelHealth(item); const ready = isSalesChannelOperational(item); return <tr key={item.id}><td><Provider provider={item.provider} /></td><td><strong>{item.displayName}</strong><small>{item.integrationMode}</small></td><td>{item.branchName}</td><td><code>{item.externalStoreId || "—"}</code></td><td><Status variant={health.key} label={health.label} title={item.lastError ?? undefined} /></td><td><strong>{item.mappingCount}</strong><small>{item.pendingOrders} pedido(s) pendente(s)</small></td><td>{dateTime(item.lastSyncedAt)}</td><td><div className="catalog-actions"><button className="catalog-action catalog-action--info" onClick={() => onDiagnostics(item)}><span className="material-symbols-rounded">monitor_heart</span>Diagnóstico</button>{permission?.canManage && <button className="catalog-action catalog-action--success" disabled={!ready || acting === item.id} title={ready ? "Sincronizar catálogo, preços e estoque" : "Disponível após autorização oficial"} onClick={() => void onSync(item.id)}><span className="material-symbols-rounded">sync</span>Sincronizar</button>}{permission?.canUpdate && <button className="catalog-action catalog-action--edit" onClick={() => onEdit(item)}><span className="material-symbols-rounded">edit</span>Configurar</button>}{permission?.canDelete && <button className="catalog-action catalog-action--danger" onClick={() => onRemove(item)}><span className="material-symbols-rounded">link_off</span>Remover</button>}</div></td></tr>; })}</tbody></table>{!data.rows.length && <Empty icon="hub" title="Nenhuma conexão encontrada" text="Prepare uma loja ou altere os filtros para visualizar conexões." />}</DataPanel>;
}
...
function MappingModal({ editing, connections, form, setForm, busy, onClose, onSubmit }: { editing: Mapping | null; connections: ConnectionOption[]; form: MappingForm; setForm: (value: MappingForm) => void; busy: boolean; onClose: () => void; onSubmit: (event: FormEvent) => void }) {
  ...
  useDialogAccessibility(true, onClose);
  useEffect(() => { const timer = window.setTimeout(async () => { setLoading(true); try { const result = await executeQuery(queryRef(dc, "SalesChannelProductOptions", { term: term.trim(), limit: 30, requestKey: crypto.randomUUID() })); setProducts((result.data as { _select?: ProductOption[] })._select ?? []); } catch (error) { console.error(error); } finally { setLoading(false); } }, term ? 250 : 0); return () => window.clearTimeout(timer); }, [term]);
  return <div className="catalog-backdrop"><section className="catalog-modal master-modal channel-modal" role="dialog" aria-modal="true" aria-label="Vincular produto"><header><div><span className="eyebrow">Catálogo integrado</span><h2>{editing ? "Configurar vínculo" : "Vincular produto"}</h2></div><button onClick={onClose} aria-label="Fechar">×</button></header><form onSubmit={onSubmit}><div className="channel-modal-body"><div className="master-form-grid"><label><span>Conexão</span><select value={form.connectionId} disabled={Boolean(editing)} onChange={(event) => setForm({ ...form, connectionId: event.target.value })} required><option value="">Selecione</option>{connections.map((item) => <option key={item.id} value={item.id}>{salesChannelProviderLabel(item.provider)} · {item.displayName} · {item.branchName}</option>)}</select></label><label className="channel-product-picker"><span>Produto Insight Pad</span><input value={term} disabled={Boolean(editing)} onFocus={() => setPickerOpen(true)} onChange={(event) => { setTerm(event.target.value); setPickerOpen(true); setForm({ ...form, productId: "" }); }} placeholder="Nome, código ou EAN" autoComplete="off" role="combobox" aria-expanded={pickerOpen} aria-controls="channel-product-results" />{!editing && pickerOpen && <div id="channel-product-results" className="channel-product-results" role="listbox">{loading ? <small>Pesquisando...</small> : products.map((item) => <button type="button" role="option" aria-selected={form.productId === item.id} key={item.id} onClick={() => { setForm({ ...form, productId: item.id }); setTerm(item.name); setPickerOpen(false); }}><strong>{item.name}</strong><small>{item.internalCode || item.ean || "Sem código"}</small></button>)}</div>}</label><label><span>ID no parceiro</span><input value={form.externalProductId} maxLength={160} onChange={(event) => setForm({ ...form, externalProductId: event.target.value })} required /></label><label><span>Nome no parceiro <small>(opcional)</small></span><input value={form.externalProductName} maxLength={240} onChange={(event) => setForm({ ...form, externalProductName: event.target.value })} /></label><label className="channel-toggle"><input type="checkbox" checked={form.syncPrice} onChange={(event) => setForm({ ...form, syncPrice: event.target.checked })} /><span>Sincronizar preço automaticamente?</span></label><label className="channel-toggle"><input type="checkbox" checked={form.syncStock} onChange={(event) => setForm({ ...form, syncStock: event.target.checked })} /><span>Sincronizar estoque automaticamente?</span></label>{editing && <label className="channel-toggle"><input type="checkbox" checked={form.enabled} onChange={(event) => setForm({ ...form, enabled: event.target.checked })} /><span>Manter este vínculo ativo?</span></label>}</div></div><footer><button type="button" className="catalog-modal-cancel" onClick={onClose} disabled={busy}>Cancelar</button><button className={`catalog-primary catalog-modal-submit ${editing ? "catalog-modal-submit--edit" : "catalog-modal-submit--create"}`} disabled={busy || !form.productId}>{busy ? "Salvando..." : editing ? "Salvar vínculo" : "Vincular produto"}</button></footer></form></section></div>;
}

function RemoveDialog({ connection, busy, onClose, onConfirm }: { connection: boolean; busy: boolean; onClose: () => void; onConfirm: () => void }) { useDialogAccessibility(true, onClose); return <div className="catalog-backdrop"><section className="catalog-confirm" role="alertdialog" aria-modal="true" aria-label="Confirmar remoção"><span className="material-symbols-rounded">{connection ? "link_off" : "delete"}</span><h2>{connection ? "Remover conexão?" : "Remover vínculo?"}</h2><p>{connection ? "Vínculos e filas pendentes também serão desativados. O histórico continuará preservado para auditoria." : "O produto deixará as próximas sincronizações, com histórico preservado."}</p><footer><button className="catalog-modal-cancel" onClick={onClose} disabled={busy}>Cancelar</button><button className="catalog-action catalog-action--danger catalog-modal-submit" onClick={onConfirm} disabled={busy}>{busy ? "Removendo..." : "Confirmar remoção"}</button></footer></section></div>; }
function Diagnostics({ connection, onClose }: { connection: Connection; onClose: () => void }) { useDialogAccessibility(true, onClose); const health = salesChannelHealth(connection); return <div className="catalog-backdrop"><section className="catalog-modal master-modal channel-diagnostics-modal" role="dialog" aria-modal="true" aria-label="Diagnóstico"><header><div><span className="eyebrow">Saúde da integração</span><h2>{connection.displayName}</h2></div><button onClick={onClose} aria-label="Fechar">×</button></header><div className="channel-diagnostics-body"><div className="channel-diagnostics-hero"><Provider provider={connection.provider} /><Status variant={health.key} label={health.label} /></div><dl><div><dt>Autorização</dt><dd>{connection.authorizationStatus}</dd></div><div><dt>Modo</dt><dd>{connection.integrationMode}</dd></div><div><dt>Webhook</dt><dd>{connection.webhookStatus}</dd></div><div><dt>Falhas consecutivas</dt><dd>{connection.consecutiveFailures}</dd></div><div><dt>Última verificação</dt><dd>{dateTime(connection.lastHealthCheckAt)}</dd></div><div><dt>Último sucesso</dt><dd>{dateTime(connection.lastSuccessAt)}</dd></div><div><dt>Expiração</dt><dd>{dateTime(connection.tokenExpiresAt)}</dd></div><div><dt>ID da requisição</dt><dd><code>{connection.lastRequestId || "—"}</code></dd></div></dl>{connection.lastError && <div className="channel-diagnostics-error"><span className="material-symbols-rounded">error</span><div><strong>Última falha</strong><p>{connection.lastError}</p></div></div>}<div className="channel-diagnostics-security"><span className="material-symbols-rounded">lock</span><p>Somente metadados operacionais são exibidos. Tokens, segredos e payloads brutos não chegam ao navegador.</p></div></div><footer><button className="catalog-modal-cancel" onClick={onClose}>Fechar</button></footer></section></div>; }
