...
export function SalesChannelsPage() {
  ...
  async function sync(connectionId: string) {
    ...
    if (profile !== "RESTAURANT") {
      setNotice({ type: "error", text: profile === "GROCERY" ? "O envio de catálogo Mercado aguarda a integração oficial da Item API do iFood." : "Confirme o tipo de catálogo em Configurar antes de sincronizar." });
      return;
    }
...
function Diagnostics({ connection, onClose }: { connection: Connection; onClose: () => void }) { useDialogAccessibility(true, onClose); const health = salesChannelHealth(connection); return <div className="catalog-backdrop"><section className="catalog-modal master-modal channel-diagnostics-modal" role="dialog" aria-modal="true" aria-label="Diagnóstico"><header><div><span className="eyebrow">Saúde da integração</span><h2>{connection.displayName}</h2></div><button onClick={onClose} aria-label="Fechar">×</button></header><div className="channel-diagnostics-body"><div className="channel-diagnostics-hero"><Provider provider={connection.provider} /><Status variant={health.key} label={health.label} /></div><dl><div><dt>Autorização</dt><dd>{connection.authorizationStatus}</dd></div><div><dt>Modo</dt><dd>{connection.integrationMode}</dd></div><div><dt>Catálogo</dt><dd>{connection.catalogProfile === "GROCERY" ? "Mercado: publicação ainda não disponível" : connection.catalogProfile === "RESTAURANT" ? "Atualização de preço e disponibilidade" : "Tipo de loja ainda não confirmado"}</dd></div><div><dt>Webhook</dt><dd>{connection.webhookStatus}</dd></div><div><dt>Falhas consecutivas</dt><dd>{connection.consecutiveFailures}</dd></div><div><dt>Última verificação</dt><dd>{dateTime(connection.lastHealthCheckAt)}</dd></div><div><dt>Último sucesso</dt><dd>{dateTime(connection.lastSuccessAt)}</dd></div><div><dt>Expiração</dt><dd>{dateTime(connection.tokenExpiresAt)}</dd></div><div><dt>ID da requisição</dt><dd><code>{connection.lastRequestId || "—"}</code></dd></div></dl>{connection.lastError && <div className="channel-diagnostics-error"><span className="material-symbols-rounded">error</span><div><strong>Última falha</strong><p>{connection.lastError}</p></div></div>}<div className="channel-diagnostics-security"><span className="material-symbols-rounded">lock</span><p>Somente metadados operacionais são exibidos. Tokens, segredos e payloads brutos não chegam ao navegador.</p></div></div><footer><button className="catalog-modal-cancel" onClick={onClose}>Fechar</button></footer></section></div>; }
