...
type ProductOption = { id: string; name: string; internalCode: string; ean?: string | null; scaleCode?: string | null };
type Filters = { status: string; provider: string; branchId: string; dateFrom: string; dateTo: string };
type Item = { id: string; productId?: string | null; mappingStatus: string; externalItemId?: string | null; externalProductId?: string | null; ean?: string | null; name: string; quantity: number; unitPriceCents: string; totalCents: string; observation?: string | null };
...
type Order = {
  id: string; providerOrderId: string; displayCode: string; provider: SalesChannelProvider; catalogProfile: "UNVERIFIED" | "RESTAURANT" | "GROCERY"; connectionName: string; authorizationStatus: string; grocerySeparationStarted?: boolean;
...
function OrderItemMappingDialog({ order, item, busy, onClose, onConfirm }: { order: Order; item: Item; busy: boolean; onClose: () => void; onConfirm: (productId: string) => void }) {
  ...
  useEffect(() => { let cancelled = false; const timer = window.setTimeout(async () => { setLoading(true); try { const result = await executeQuery(queryRef(dc, "SalesChannelProductOptions", { term: term.trim(), limit: 30, requestKey: crypto.randomUUID() })); if (!cancelled) setProducts((result.data as { _select?: ProductOption[] })._select ?? []); } catch (error) { console.error(error); if (!cancelled) setProducts([]); } finally { if (!cancelled) setLoading(false); } }, term ? 250 : 0); return () => { cancelled = true; window.clearTimeout(timer); }; }, [term]);
  return <div className="catalog-backdrop"><section className="catalog-modal master-modal channel-mapping-modal" role="dialog" aria-modal="true" aria-label="Vincular item ao produto interno"><header><div><span className="eyebrow">Pedido #{order.displayCode}</span><h2>Vincular “{item.name}”</h2></div><button onClick={onClose} aria-label="Fechar" disabled={busy}>×</button></header><div className="channel-reject-body"><p>Escolha o produto que movimentará o estoque nesta e nas próximas ocorrências do mesmo item. O vínculo será auditado e o pedido reconciliado automaticamente.</p><label><span>Pesquisar produto</span><input autoFocus value={term} onChange={(event) => { setTerm(event.target.value); setSelected(""); }} maxLength={160} placeholder="Nome, código interno ou EAN" disabled={busy} /></label><div className="channel-product-results" role="listbox" aria-label="Produtos encontrados">{loading ? <small>Pesquisando...</small> : products.length ? products.map((product) => <button type="button" role="option" aria-selected={selected === product.id} key={product.id} onClick={() => setSelected(product.id)}><strong>{product.name}</strong><small>{product.internalCode}{product.ean ? ` · EAN ${product.ean}` : product.scaleCode ? ` · Balança ${product.scaleCode}` : ""}</small></button>) : <small>Nenhum produto encontrado.</small>}</div></div><footer><button className="catalog-modal-cancel" onClick={onClose} disabled={busy}>Cancelar</button><button className="catalog-primary catalog-modal-submit" disabled={busy || !selected} onClick={() => onConfirm(selected)}>{busy ? "Vinculando..." : "Vincular e reconciliar"}</button></footer></section></div>;
}
