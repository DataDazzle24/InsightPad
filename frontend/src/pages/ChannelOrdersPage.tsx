...
type Order = {
  id: string; providerOrderId: string; displayCode: string; provider: SalesChannelProvider; connectionName: string; authorizationStatus: string;
...
const actionLabels: Record<Action, string> = { ACCEPT: "aceite", REJECT: "recusa", COMPLETE: "conclusão", CANCEL: "cancelamento" };
...
export function ChannelOrdersPage() {
  ...
  async function queue(order: Order, action: Action, reason = "") {
    setActing(order.id);
    try {
      const result = await executeMutation(mutationRef(dc, "QueueSalesChannelOrderAction", { id: order.id, action, reason, expectedVersion: order.version }));
  ...
  return <section className="catalog-page channel-orders-page">
    ...
    {confirm && <ActionDialog action={confirm.action} order={confirm.order} busy={acting === confirm.order.id} onClose={() => setConfirm(null)} onConfirm={(reason) => void queue(confirm.order, confirm.action, reason)} />}
...
function OrderStatus({ order }: { order: Order }) { const pending = ["QUEUED", "RETRY", "PROCESSING"].includes(order.commandStatus); const error = order.commandStatus === "ERROR"; return <div className="channel-order-status-stack"><span className={`channel-status channel-status--${order.status.toLowerCase()}`}><i />{statusLabels[order.status]}</span>{pending && <span className="channel-command-state"><span className="material-symbols-rounded">sync</span>{order.pendingAction ? `${actionLabels[order.pendingAction as Action] ?? order.pendingAction} em processamento` : "Ação em processamento"}</span>}{error && <span className="channel-command-state channel-command-state--error"><span className="material-symbols-rounded">error</span>Falha no envio</span>}</div>; }
function OrderActions({ order, canUpdate, busy, expanded, onExpand, onAction }: { order: Order; canUpdate: boolean; busy: boolean; expanded: boolean; onExpand: () => void; onAction: (action: Action) => void }) { const authorized = order.authorizationStatus === "AUTHORIZED"; const pendingCommand = ["QUEUED", "RETRY", "PROCESSING"].includes(order.commandStatus); const disabled = busy || pendingCommand || !authorized; return <div className="catalog-actions"><button className="catalog-action catalog-action--info" onClick={onExpand}><span className="material-symbols-rounded">{expanded ? "expand_less" : "visibility"}</span>{expanded ? "Fechar" : "Detalhes"}</button>{canUpdate && order.status === "PENDING" && <><button className="catalog-action catalog-action--success" disabled={disabled} title={!authorized ? "Canal não autorizado" : undefined} onClick={() => onAction("ACCEPT")}><span className="material-symbols-rounded">check</span>Aceitar</button><button className="catalog-action catalog-action--danger" disabled={disabled} onClick={() => onAction("REJECT")}><span className="material-symbols-rounded">close</span>Recusar</button></>}{canUpdate && order.status === "ACCEPTED" && <><button className="catalog-action catalog-action--success" disabled={disabled} onClick={() => onAction("COMPLETE")}><span className="material-symbols-rounded">task_alt</span>Concluir</button><button className="catalog-action catalog-action--danger" disabled={disabled} onClick={() => onAction("CANCEL")}><span className="material-symbols-rounded">cancel</span>Cancelar</button></>}</div>; }
...
function ActionDialog({ action, order, busy, onClose, onConfirm }: { action: Action; order: Order; busy: boolean; onClose: () => void; onConfirm: (reason: string) => void }) { const [reason, setReason] = useState(""); useDialogAccessibility(true, onClose); const requiresReason = action === "REJECT" || action === "CANCEL"; const valid = !requiresReason || reason.trim().length >= 5; return <div className="catalog-backdrop"><section className="catalog-modal master-modal channel-reject-modal" role="dialog" aria-modal="true" aria-label={`${actionLabels[action]} do pedido`}><header><div><span className="eyebrow">Pedido #{order.displayCode}</span><h2>Confirmar {actionLabels[action]}</h2></div><button onClick={onClose} aria-label="Fechar">×</button></header><div className="channel-reject-body"><p>A solicitação será enviada ao parceiro e o status só mudará após a confirmação oficial.</p>{requiresReason && <label><span>Motivo obrigatório</span><textarea autoFocus value={reason} onChange={(event) => setReason(event.target.value)} maxLength={500} placeholder="Informe pelo menos 5 caracteres" /></label>}</div><footer><button className="catalog-modal-cancel" onClick={onClose} disabled={busy}>Cancelar</button><button className={`catalog-action catalog-modal-submit ${action === "REJECT" || action === "CANCEL" ? "catalog-action--danger" : "catalog-action--success"}`} disabled={!valid || busy} onClick={() => onConfirm(reason.trim())}>{busy ? "Enviando..." : "Confirmar solicitação"}</button></footer></section></div>; }
